...
pub(crate) async fn resolve_frame_context(
    ...
    if frame_id.is_none() {
        FRAME_PRIMARY_FAST_PATH_HITS.set(FRAME_PRIMARY_FAST_PATH_HITS.get().saturating_add(1));
...
async fn read_main_frame(page: &Arc<Page>) -> Result<FrameId, RubError> {
    page.mainframe()
        .await
...
async fn collect_frame_inventory(page: &Arc<Page>) -> Result<Vec<FrameInventoryEntry>, RubError> {
    ...
    FRAME_INVENTORY_ENTRY_STEPS.set(
        FRAME_INVENTORY_ENTRY_STEPS
            .get()
        ...
        raw_entries.push(FrameContextInfo {
            ...
            same_origin_accessible: if needs_same_origin_probe {
                FRAME_SAME_ORIGIN_PROBE_REQUESTS
                    .set(FRAME_SAME_ORIGIN_PROBE_REQUESTS.get().saturating_add(1));
...
fn frame_runtime_metrics_snapshot() -> FrameRuntimeMetrics {
    FrameRuntimeMetrics {
        primary_fast_path_hits: FRAME_PRIMARY_FAST_PATH_HITS.get(),
        inventory_collections: FRAME_INVENTORY_COLLECTIONS.get(),
        inventory_entry_steps: FRAME_INVENTORY_ENTRY_STEPS.get(),
        same_origin_probe_requests: FRAME_SAME_ORIGIN_PROBE_REQUESTS.get(),
    }
}

#[cfg(test)]
fn reset_frame_runtime_metrics() {
    FRAME_PRIMARY_FAST_PATH_HITS.set(0);
    FRAME_INVENTORY_COLLECTIONS.set(0);
    FRAME_INVENTORY_ENTRY_STEPS.set(0);
    FRAME_SAME_ORIGIN_PROBE_REQUESTS.set(0);
}

fn build_lineage(current_frame_id: &str, inventory: &[FrameInventoryEntry]) -> Vec<String> {
    let parent_by_frame = inventory_parent_lookup(inventory);
    collect_frame_lineage(current_frame_id, |frame_id| {
        parent_by_frame
            .get(frame_id)
            .copied()
            .flatten()
...
fn inventory_parent_lookup(inventory: &[FrameInventoryEntry]) -> HashMap<&str, Option<&str>> {
    inventory
        .iter()
        .map(|entry| {
            (
...
mod tests {
    ...
    fn build_lineage_collects_current_to_root_order() {
        let inventory = vec![
            FrameInventoryEntry {
                index: 0,
                frame: sample_frame_context("main", Some("main")),
                is_current: true,
                is_primary: true,
            },
            FrameInventoryEntry {
    ...
    fn frame_runtime_metrics_capture_probe_plan_baseline() {
        ...
        super::FRAME_INVENTORY_COLLECTIONS.set(1);
        super::FRAME_INVENTORY_ENTRY_STEPS.set(ordered.len().try_into().unwrap_or(u64::MAX));
        let probe_requests = ordered
            .iter()
            .filter(|frame_id| same_origin_probe_required(frame_id.as_ref(), &main_frame_id))
            .count() as u64;
        super::FRAME_SAME_ORIGIN_PROBE_REQUESTS.set(probe_requests);
    ...
    fn frame_runtime_metrics_snapshot_tracks_primary_fast_path_hits() {
        reset_frame_runtime_metrics();
        super::FRAME_PRIMARY_FAST_PATH_HITS.set(2);
