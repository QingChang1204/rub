...
fn atomic_fill_rollback_rejects_editor_safe_surface_in_v1() {
    ...
    let envelope =
        build_atomic_rollback_command_for_resolved_target(&element, "type_editor_text", "hello")
...
fn resolve_multiple_references_in_one_string() {
    let completed = mock_completed_steps();
    let mut args =
        json!({ "msg": "Title: {{steps[0].result.fields.title}}, Value: {{prev.result.value}}" });
    resolve_step_references(&mut args, &completed, 2).unwrap();
...
fn resolve_whole_placeholder_preserves_bool() {
    let completed = vec![json!({ "action": { "command": "exec" }, "result": { "done": true } })];
...
fn parse_pipe_spec_rejects_duplicate_labels() {
    ...
    let spec = NormalizedJsonSpec::from_raw_str(spec, "pipe").expect("duplicate label spec parses");
