...
use crate::request_correlation::RequestCorrelation;
...
impl IpcServer {
    ...
    pub async fn bind(socket_path: &Path) -> Result<Self, io::Error> {
...
impl IpcConnection {
    ...
    pub async fn write_response_for_request(
        ...
        response
            ...
            .map_err(|envelope| io::Error::new(io::ErrorKind::InvalidData, envelope.message))?;
        response
        ...
        Err(error) => match error.downcast::<io::Error>() {
            Ok(io_error) => {
                let reason = match io_error.kind() {
                    io::ErrorKind::UnexpectedEof => "partial_ndjson_frame",
                    io::ErrorKind::InvalidData
                        if crate::codec::is_oversized_frame_io_error(io_error.as_ref()) =>
                    {
                        "oversized_ndjson_frame"
                    }
                    io::ErrorKind::InvalidData => "invalid_ndjson_frame",
                    _ => "ipc_read_failure",
                };
                ErrorEnvelope::new(
